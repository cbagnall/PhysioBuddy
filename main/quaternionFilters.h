...
{
private:
  static constexpr float GyroMeasError = PI * (40.0f / 180.0f);
  
  // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
  static constexpr float GyroMeasDrift = PI * (0.0f  / 180.0f);
  ...
  static constexpr float beta = sqrt(3.0f / 4.0f) * GyroMeasError;   // Compute beta
  // Compute zeta, the other free parameter in the Madgwick scheme usually
  // set to a small or zero value
  
  static constexpr float zeta = sqrt(3.0f / 4.0f) * GyroMeasDrift;
...
public:
  ...
  const inline float * getQ() { return q; };
